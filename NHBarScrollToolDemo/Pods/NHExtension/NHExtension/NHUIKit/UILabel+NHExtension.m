...
- (void)nh_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings tapClicked:(void (^) (NSString *string , NSRange range , NSInteger index))tapClick
{
    [self nh_getRangesWithStrings:strings];
                              ...
                              tapClicked:(void (^)(NSString *, NSString *, NSInteger))tapClick
{
    ...
    [self nh_getRangesWithStrings:strings];
    [self nh_getWithHideArray:hideString];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self nh_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        ...
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(nh_attributeTapReturnString:range:index:)]) {
            [weakSelf.delegate nh_attributeTapReturnString:string range:range index:index];
        }
        
        if (self.isTapEffect) {
            
            [self nh_saveEffectDicWithRange:range];
            
            [self nh_tapEffectWithStatus:YES];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isTapAction) {
        if ([self nh_getTapFrameWithTouchPoint:point result:nil]) {
            return self;
        }
    }
...
- (BOOL)nh_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self nh_getLineBounds:line point:linePoint];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    if (self.isTapEffect) {
        ...
        [self performSelectorOnMainThread:@selector(nh_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
...
- (CGRect)nh_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
...
- (void)nh_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void)nh_saveEffectDicWithRange:(NSRange)range
{
...
- (void)nh_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf nh_getStringWithRange:range]];
...
- (void)nh_getWithHideArray:(NSArray <NSString *>  *)hideArr{
    if (self.attributedText == nil) {
        return;
    }
...
- (NSString *)nh_getStringWithRange:(NSRange)range
{
